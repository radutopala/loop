...
// input (see the onData handler). A reply spans the whole onData payload, so
// each pattern is anchored ^…$.
const TERMINAL_REPORT_REPLIES: RegExp[] = [
  /^\x1b\[\d+;\d+R$/, // DSR — Cursor Position Report (ESC[row;colR)
  /^\x1b\][0-9]+;.*?(?:\x07|\x1b\\)$/, // OSC color reports (ESC]4;n;…  ESC]10/11/12;… terminated by BEL or ST)
  /^\x1b\[\?\d+;\d+\$y$/, // DECRPM — mode report (ESC[?mode;value$y)
  /^\x1b\[\?[\d;]*c$/, // DA — Device Attributes report (ESC[?…c)
];

// isTerminalReportReply reports whether `data` is a terminal capability-query
// reply auto-generated by xterm.js (color/mode/cursor/device probes) rather
// than genuine keyboard input.
export function isTerminalReportReply(data: string): boolean {
  return TERMINAL_REPORT_REPLIES.some((re) => re.test(data));
}
...
export function useXTerminal({
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      term.onData((data) => {
        // Drop terminal capability-query *replies* that xterm.js auto-generates.
        // Modern Ink/React TUIs (Claude Code included) probe colors and modes on
        // startup; xterm answers, and those answers arrive here as if typed. If
        // forwarded to the PTY an idle shell echoes them as garbage — and they
        // resurface on reattach when the session ring buffer is replayed into
        // xterm and re-parsed. None of these are ever real keyboard input.
