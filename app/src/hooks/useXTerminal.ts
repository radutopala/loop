...
export function useXTerminal({
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Shift+Enter → backslash + CR. xterm.js otherwise sends a plain \r
      // for both Enter and Shift+Enter, so TUIs cannot tell them apart. The
      // `\<Enter>` sequence is what Claude Code's /terminal-setup configures
      // iTerm2 to emit (and what bash readline reads as a line continuation),
      // so both interactive TUIs and shells insert a newline instead of
      // submitting. The handler must return false for both keydown AND
      // keypress — xterm fires onData from keypress for Enter, so blocking
      // only keydown still leaks a plain \r through.
      term.attachCustomKeyEventHandler((event) => {
        if (event.key === "Enter" && event.shiftKey) {
          if (event.type === "keydown") onInput("\\\r");
          return false;
        }
        return true;
      });
