...
import { bandColor, EmptyState, ErrorState, LoadingState, metricLabel, SectionHelp } from "./shared";
...
export function WhatifTab({ target, loading, error, result, colors, fontSizes }: WhatifTabProps) {
  if (!target && !loading) {
    return <EmptyState text="No simulation yet. Click a tile in Overview or Diagnostics, then 'Simulate delete'." colors={colors} fontSizes={fontSizes} />;
  }
  ...
  return (
    <div style={{ padding: 12, display: "flex", flexDirection: "column", gap: 12 }}>
      <SectionHelp colors={colors}>
        Simulates a mutation against a clone of the in-memory graph and recomputes all five metrics on the shadow. The real graph isn&apos;t touched. Use it to A/B refactor candidates before
        committing — positive <b>delta</b> means the change improves the signal; the per-metric breakdown shows which dimensions move. Predictions are exact for this snapshot, not closed-form
        approximations.
      </SectionHelp>
      <div style={{ fontSize: 11, color: colors.textDim, textTransform: "uppercase", letterSpacing: 0.5 }}>Mutation</div>
      <div style={{ fontFamily: fonts.mono, fontSize: 12, color: colors.textLight, wordBreak: "break-all" }}>delete {target}</div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px", border: `1px solid ${colors.border}`, borderRadius: 6, background: colors.surface }}>
        <div style={{ flex: 1, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 24, fontFamily: fonts.mono, color: bandColor(result.baseline_signal) }}>{result.baseline_signal}</div>
        </div>
        <div style={{ fontSize: 18, color: colors.textDim }}>→</div>
        <div style={{ flex: 1, textAlign: "center" }}>
          <div style={{ fontSize: 11, color: colors.textDim }}>predicted</div>
          <div style={{ fontSize: 24, fontFamily: fonts.mono, color: bandColor(result.predicted_signal) }}>{result.predicted_signal}</div>
        </div>
        <div style={{ flex: 1, textAlign: "center" }}>
      ...
      <div>
        <div style={{ fontSize: 11, color: colors.textDim, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>Per-metric breakdown</div>
          ...
          {result.baseline_metrics.map((bm) => {
            ...
            return (
              <div key={bm.name} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ width: 60, textAlign: "right", color: dsColor }}>{ds > 0 ? `+${ds.toFixed(3)}` : ds.toFixed(3)}</div>
              </div>
            );
          })}
        </div>
