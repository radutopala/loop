...
export function WorkflowsLayoutPanel({ channelId }: WorkflowsLayoutPanelProps) {
  ...
  const onEvent = useCallback((evt: { type: string; data?: unknown }) => {
      ...
      >
        ...
        >
          {wf.displayedRuns.map((r) => (
            <WorkflowRunRow key={r.id} run={r} isSelected={r.id === wf.selectedRunId} colors={colors} onClick={() => wf.selectRun(r.id)} testId={`workflow-run-row-${r.id}`} />
          ))}
