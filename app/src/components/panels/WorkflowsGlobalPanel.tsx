...
export const WorkflowsGlobalPanel = forwardRef<WorkflowsGlobalPanelHandle, WorkflowsGlobalPanelProps>(function WorkflowsGlobalPanel(
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        e.preventDefault();
        if (wf.showStartDialog) {
          wf.setShowStartDialog(false);
        } else {
          onClose();
        }
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button onClick={onClose} title="Close panel" style={headerBtnStyle} onMouseEnter={(e) => hoverIn(e, colors)} onMouseLeave={(e) => hoverOut(e, colors)}>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        >
          <WorkflowDefList grouped={wf.groupedDefinitions} selectedName={wf.selectedWorkflowName} onSelect={wf.setSelectedWorkflowName} onRun={wf.handleRunWorkflow} colors={colors} />
          <div
          ...
          >
            ...
            {wf.loadingMore && <div style={{ padding: 12, color: colors.textDim, fontSize: 11, textAlign: "center" }}>Loading more…</div>}
            {!wf.hasMore && !wf.selectedWorkflowName && wf.runs.length > 0 && <div style={{ padding: 12, color: colors.textDim, fontSize: 11, textAlign: "center", opacity: 0.6 }}>End of history</div>}
            {wf.displayedRuns.length === 0 && (
              <div style={{ padding: 16, color: colors.textDim, fontSize: 12, textAlign: "center" }}>{wf.selectedWorkflowName ? `No runs for ${wf.selectedWorkflowName}` : "No workflow runs"}</div>
            )}
          </div>
        </div>
