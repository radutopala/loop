...
import { deleteWorktree } from "../../api/channels";
...
export function WorktreesPanel({ channelId, isWorktree, hasBranch, onImportWorktree, onSelectThread }: WorktreesPanelProps) {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (!target.closest("[data-confirm-popover]")) setConfirmingId(null);
    };
  ...
  const handleDelete = async (wt: WorktreeInfo) => {
    ...
    try {
      await deleteWorktree(wt.thread_id);
      await loadWorktrees();
    } catch {
      // ignore
    } finally {
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    {confirmingId === wt.thread_id && (
                      ...
                      >
                        ...
                        <button
                          onClick={() => { setConfirmingId(null); handleDelete(wt); }}
                          style={{
