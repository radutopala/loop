    ...
    >
      ...
      >
        <div style={{ fontSize: 14, fontWeight: 600, color: colors.text, marginBottom: 4 }}>
          {isWorktree ? "Rename Worktree" : "Rename Thread"}
        </div>
        {isWorktree && (
        ...
        <div style={{ display: "flex", gap: 6, justifyContent: "flex-end", marginTop: 12 }}>
          ...
          <button
            ...
            onClick={submit}
            disabled={!canSubmit}
            style={{ padding: "6px 14px", borderRadius: 6, border: "none", background: canSubmit ? colors.active : colors.border, color: colors.white, fontSize: 13, cursor: canSubmit ? "pointer" : "default", opacity: canSubmit ? 1 : 0.6 }}
          >
            Rename
          </button>
        </div>
