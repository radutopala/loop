...
| `container_memory_mb` | `2048` | Memory limit per container (MB); `0` for no cap |
...
| `claude_model` | `"claude-sonnet-5"` | Claude model (e.g. `"claude-opus-5-5"`, `"claude-fable-5-1"`, `"claude-opus-4-8"`). Overridable per channel from the chat composer |
...
| `chat_components` | **Merged** with global and built-ins; project overrides by name |
