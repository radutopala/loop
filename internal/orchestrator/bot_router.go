...
func (r *BotRouter) HandleIncomingMessage(ctx context.Context, channelID, authorID, content, mode string, worktree bool, branch, worktreePath string) {
	if b := r.botForChannel(ctx, channelID); b != nil {
		b.HandleIncomingMessage(ctx, channelID, authorID, content, mode, worktree, branch, worktreePath)
	}
}
