...
func (s *EventsHubSuite) TestBroadcastChannelUpdated() {
	hub := NewEventsHub(testLogger())
	...
	hub.BroadcastChannelUpdated(events.ChannelUpdatedData{
		ChannelID:     "ch-1",
		Branch:        "feat/x",
		Commit:        "abc1234",
		DiffAdditions: 3,
	...
	var evt Event
	require.NoError(s.T(), json.Unmarshal(msg, &evt))
	require.Equal(s.T(), "channel.updated", evt.Type)
	require.Equal(s.T(), "ch-1", evt.ChannelID)
	data, ok := evt.Data.(map[string]any)
	require.True(s.T(), ok)
	require.Equal(s.T(), "feat/x", data["branch"])
...
func (s *EventsHubSuite) TestBroadcastAskResolved() {
	hub := NewEventsHub(testLogger())
	...
	var evt Event
	require.NoError(s.T(), json.Unmarshal(msg, &evt))
	require.Equal(s.T(), "agent.ask_resolved", evt.Type)
	require.Equal(s.T(), "ch-1", evt.ChannelID)
}

func (s *EventsHubSuite) TestBroadcastPlanResolved() {
	hub := NewEventsHub(testLogger())
	...
	hub.BroadcastPlanResolved("ch-1")
	...
	var evt Event
	require.NoError(s.T(), json.Unmarshal(msg, &evt))
	require.Equal(s.T(), "agent.plan_resolved", evt.Type)
	require.Equal(s.T(), "ch-1", evt.ChannelID)
}
...
func (s *EventsHubSuite) TestBroadcastMessageDeleted() {
	hub := NewEventsHub(testLogger())
	...
	hub.BroadcastMessageDeleted("ch-1", events.MessageDeletedData{MsgID: "msg-q"})
	...
	var evt Event
	require.NoError(s.T(), json.Unmarshal(msg, &evt))
	require.Equal(s.T(), "message.deleted", evt.Type)
	require.Equal(s.T(), "ch-1", evt.ChannelID)
}
