...
Reviewing for **recall** at high effort: catch every real bug. Catching real bugs matters more than avoiding false positives — a missed bug ships. Err on the side of surfacing. Do not fix anything yourself — the user triages comments from the Review panel.

## Phase 0 — Gather the diff

Run ` + "`git diff @{upstream}...HEAD`" + ` (or ` + "`git diff main...HEAD`" + ` / ` + "`git diff HEAD~1`" + ` if there is no upstream) to get the unified diff under review. If there are uncommitted changes, or the range diff is empty, also run ` + "`git diff HEAD`" + ` and include the working-tree changes in scope. Treat this diff as the review scope.

## Phase 1 — Find candidates (5 angles, up to 8 each)

Run **5 independent finder angles** via the Agent tool, concurrently in a single message. Each surfaces **up to 8 candidate findings**. Do NOT let one angle's conclusions suppress another's — if two angles flag the same line for different reasons, record both.
...
## Output — emit comments

Aggregate the surviving findings (CONFIRMED + PLAUSIBLE from Phase 2, plus the sweep from Phase 3), ranked most-severe first, **at most 15 total**. For each, emit exactly one block:
...
side defaults to "RIGHT" (added/modified lines). Use side="LEFT" only when commenting on a line removed from the base. Do not emit blocks for style nits, formatting, or matters of taste — only problems that should block the PR. If nothing survives, emit no blocks.`
