...
// Run executes the review prompt. systemPrompt + prompt are passed
// straight through to the agent (the caller is expected to have resolved
// the configured review prompt and assembled the diff context). For each
// `<review-comment>` block the agent emits, onComment fires with the
// parsed Comment exactly once — duplicate ids that arrive in a later
// turn are ignored so reruns don't double-broadcast.
func (r *Runner) Run(ctx context.Context, channelID, dirPath, systemPrompt, prompt string, onComment func(*Comment)) (*agent.AgentResponse, error) {
	if r.Agent == nil {
