...
// makes it look handled when it is not — hence no_proxy_hosts.
...
func (s *RunnerSuite) TestDockerCLIProxyConfigCarriesNoProxyHosts() {
	const proxy = "http://host.docker.internal:3128"
	getenv := func(k string) string {
		if k == "HTTP_PROXY" || k == "HTTPS_PROXY" {
			return proxy
	...
	env := ProxyEnv(getenv, "loop-chrome-abc", "floci", "valkey")

	data := dockerCLIProxyConfig(env)
	require.Contains(s.T(), string(data), "floci")
	require.Contains(s.T(), string(data), "valkey")
	...
	require.Equal(s.T(), "", noProxyFor(rendered, "http://floci:4566/"))
...
func (s *RunnerSuite) TestRunNoProxyHostsFromConfig() {
	s.cfg.NoProxyHosts = []string{"floci", "valkey"}
	s.applyMockDefaults()
	...
	const want = "host.docker.internal,localhost,127.0.0.1,::1,172.16.0.0/12,loop-chrome-ch-1,floci,valkey"
