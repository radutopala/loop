...
func TestSummarizeToolInput(t *testing.T) {
	tests := []struct {
		...
		{"empty raw", "Bash", ``, ""},
		{"command under cap kept in full", "Bash", `{"command":"` + strings.Repeat("x", 200) + `"}`, strings.Repeat("x", 200)},
		{"command over cap truncated", "Bash", `{"command":"` + strings.Repeat("x", toolInputSummaryMax+100) + `"}`, strings.Repeat("x", toolInputSummaryMax) + "..."},
