...
func newMockChannelRows() *sqlmock.Rows {
	return sqlmock.NewRows([]string{"id", "channel_id", "guild_id", "name", "dir_path", "parent_id", "platform", "active", "session_id", "permissions", "worktree", "base_branch", "locked", "model_override", "effort_override", "fork_pending", "task_id", "description", "ticket_url", "created_at", "updated_at"})
}
...
func newMockMessageRows() *sqlmock.Rows {
	return sqlmock.NewRows([]string{"id", "chat_id", "channel_id", "msg_id", "author_id", "author_name", "content", "is_bot", "is_processed", "is_triggered", "is_running", "priority", "mode", "created_at", "kind", "chain_position", "tool_use_id", "tool_name", "is_error", "trigger_msg_id", "not_before"})
}
...
func addMessageRow(rows *sqlmock.Rows, id, chatID int64, channelID, msgID, authorID, authorName, content string, isBot, isProcessed int, createdAt time.Time) *sqlmock.Rows {
	return rows.AddRow(id, chatID, channelID, msgID, authorID, authorName, content, isBot, isProcessed, 0, 0, 0, "", createdAt, "message", int64(0), "", "", 0, "", int64(0))
}
