...
func (s *StoreSuite) TestLinkTaskThread() {
	...
	s.mock.ExpectExec(`INSERT INTO channels`).
		WithArgs(ch.ChannelID, ch.GuildID, ch.Name, "", ch.ParentID, "", "", "", 1, 0, "", 0, 0, sqlmock.AnyArg()).
...
func (s *StoreSuite) TestLinkTaskThreadWithPermissions() {
	...
	s.mock.ExpectBegin()
	s.mock.ExpectExec(`INSERT INTO channels`).
		WithArgs("thread-2", "", "", "", "", "", "", `{"owners":{"users":["U1"],"roles":null},"members":{"users":null,"roles":null}}`, 1, 0, "", 0, 0, sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))
	s.mock.ExpectExec(`UPDATE scheduled_tasks SET thread_id`).
