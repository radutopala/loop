...
func (s *StoreSuite) TestGetChannelWithParentID() {
	...
	rows := newMockChannelRows().
		AddRow(1, "thread1", "g1", "", "/project", "ch1", "", 1, "", "", 0, "", 0, "", "", 0, now, now)
...
func (s *StoreSuite) TestGetChannel() {
	...
	rows := newMockChannelRows().
		AddRow(1, "ch1", "g1", "test", "/home/user/project", "", "discord", 1, "sess-123", permJSON, 0, "", 0, "", "", 0, now, now)
	s.mock.ExpectQuery(`SELECT .+ FROM channels WHERE channel_id`).
		WithArgs("ch1").
		WillReturnRows(rows)
...
func (s *StoreSuite) TestGetChannelByDirPath() {
	...
	rows := newMockChannelRows().
		AddRow(1, "ch1", "g1", "loop", "/home/user/dev/loop", "", "discord", 1, "", permJSON, 0, "", 0, "", "", 0, now, now)
...
func (s *StoreSuite) TestGetChannelsByDirPath() {
	now := time.Now().UTC()
	rows := newMockChannelRows().
		AddRow(1, "ch1", "", "loop-local", "/home/user/dev/loop", "", "local", 1, "", "", 0, "", 0, "", "", 0, now, now).
		AddRow(2, "ch2", "g1", "loop-discord", "/home/user/dev/loop", "", "discord", 1, "", "", 0, "", 0, "", "", 0, now, now)
	s.mock.ExpectQuery(`SELECT .+ FROM channels WHERE dir_path`).
...
func (s *StoreSuite) TestListChannels() {
	...
	rows := newMockChannelRows().
		AddRow(1, "ch1", "g1", "alpha", "/home/user/alpha", "", "discord", 1, "sess-1", permJSON, 0, "", 0, "", "", 0, now, now).
		AddRow(2, "ch2", "g1", "beta", "/home/user/beta", "ch1", "discord", 0, "sess-2", "", 0, "", 1, "", "", 0, now, now)
...
func (s *StoreSuite) TestListChannelsErrors() {
	...
	s.mock.ExpectQuery(`SELECT .+ FROM channels ORDER BY name ASC`).WillReturnRows(
		newMockChannelRows().AddRow("not-an-int", "ch1", "g1", "test", "/home/user/project", "", "", 1, "sess-1", "", 0, "", 0, "", "", 0, time.Now().UTC(), time.Now().UTC()))
	channels, err = s.store.ListChannels(context.Background())
	require.Error(s.T(), err)
	require.Nil(s.T(), channels)
}
