...
func registerFrontendSteps(ctx *godog.ScenarioContext, tc *TestContext) {
	...
	ctx.Step(`^the row info "(branch-detail|commit|commit-detail|sync|model|status|description)" should read "([^"]*)"$`, tc.assertRowInfoLine)
	ctx.Step(`^the row info should not show "(commit|sync|model|status|description)"$`, tc.assertRowInfoLineAbsent)
