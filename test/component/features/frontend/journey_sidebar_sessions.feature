...
Feature: Active and recent sessions in the sidebar
  Above the channel tree, the sidebar lists sessions from anywhere in it:
  Active holds those waiting on you (an approval, a question, a plan) and
  then those with an agent running; Recent holds the rest that had activity
  in the last day, newest first, with how long ago. The tree follows under
  All. Each section collapses, and an empty one isn't shown.

  Scenario: A run moves a session through Active into Recent
    ...
    Then I wait for "[data-testid='sidebar-active']" to be visible
    And the element "[data-testid='sidebar-active']" should contain text "bdd-sessions"
    And the element "[data-testid='sidebar-section-all']" should be visible

    # Waiting on an approval: it stays in Active and shows the gate pill.
    When I inject a gate.approval_requested event with req_id "bdd-sessions-gate", source "chat", and target "/tmp/bdd-sessions.txt"
    Then the element "[data-testid='sidebar-active']" should contain text "gate"
    When I inject a gate.approval_resolved event with req_id "bdd-sessions-gate"
    Then the element "[data-testid='sidebar-active']" should contain text "bdd-sessions"

    # The run ends: it leaves Active and is the newest in Recent.
    When I inject an agent.status completed event
    Then the element "[data-testid='sidebar-active']" should not exist
